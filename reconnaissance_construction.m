...
img_cut = img(ligne_min:ligne_max,colonne_min:colonne_max);
img(ligne_min:ligne_max,colonne_min:colonne_max) = 0;
...
labelA_p = 1:nb_postures_base;
ListeClass_p = 1:nb_postures_base;


dpers = base(1:nb_postures_base:end,:);

[Partition] = kppv(dpers, labelA_p, dataT, 1, 1, ListeClass_p)

personne_p = Partition(1);

dpost = base(nb_postures_base*(personne_p-1)+1:nb_postures_base*personne_p,:);

[Partition] = kppv(dpost, labelA_p, dataT, 1, 1, ListeClass_p)
...
posture_proche = posture_p;figure('Name','Image tiree aleatoirement','Position',[0.2*L,0.2*H,0.8*L,0.5*H]);
...
imgfound = imread(ficF);

img(ligne_min:ligne_max,colonne_min:colonne_max) = imgfound(ligne_min:ligne_max,colonne_min:colonne_max);
        
subplot(1, 2, 2);
imagesc(img);

title({['Individu la plus proche : posture ' num2str(posture_proche) ' de ', liste_personnes{personne_proche}]}, 'FontSize', 20);
axis image;
